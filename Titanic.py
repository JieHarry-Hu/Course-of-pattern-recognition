...
for i in range(length):
    if float(age[i])<18.1:
        age[i] = 0
    elif float(age[i])>18 and float(age[i])<30.1:
        age[i] = 1
    elif float(age[i])>30 and float(age[i])<50.1:
        age[i] = 2
    elif float(age[i])>50:
        age[i] = 3
#男女先验概率
     ...
     for row in reader1:
         try:
             ...
             if float(row[4])<18.1:
                 x2 = p_0_live
                 y2 = p_0_dead
             elif float(row[4])>18 and float(row[4])<30.1:
                 x2 = p_1_live
                 y2 = p_1_dead
             elif float(row[4])>30 and float(row[4])<50.1:
                 x2 = p_2_live
                 y2 = p_2_dead
             elif float(row[4])>50:
                 x2 = p_3_live
